...
inline int _pack_pareto_sols(double* sorted_loss_values, int* nondominated_indices_buf,
                             const int& n_objectives, const int end_row_index) {
  ...
  while (n_remaining > 0) {
    ...
    const double* new_nondominated_sol =
        sorted_loss_values + (new_nondominated_index * n_objectives);
    int nondominated_count = 0;
    for (int i = 1; i < n_remaining; ++i) {
      const int& idx = nondominated_indices_buf[i];
      bool is_nondominated = false;
      const double* cand = sorted_loss_values + (idx * n_objectives);
      ...
      if (is_nondominated) nondominated_indices_buf[nondominated_count++] = idx;
    }
    std::swap_ranges(
...
double _compute_hypervolume(const double* sorted_pareto_sols, const double* ref_point,
                            int* nondominated_indices_buf, const int& n_objectives,
                            const int end_row_index) {
  double hv = 0.0;
  for (int i = 0; i < end_row_index; ++i) {
    double inclusive_hv = 1.0;
    const double* val_i = sorted_pareto_sols + (i * n_objectives);
    for (int j = 0; j < n_objectives; ++j) inclusive_hv *= ref_point[j] - val_i[j];
  ...
  if (end_row_index == 1) {
    ...
    double intersec = 1.0;
    const double* val_1 = sorted_pareto_sols + n_objectives;
    for (int j = 0; j < n_objectives; ++j)
      intersec *= ref_point[j] - std::max(sorted_pareto_sols[j], val_1[j]);
    return hv - intersec;
  }
  ...
  for (int i = 0; i < end_row_index - 1; ++i) {
    ...
    for (int j = 0; j < end_row_index_limited; ++j) {
      const double* val_j = sorted_pareto_sols + ((j + i + 1) * n_objectives);
      for (int k = 0; k < n_objectives; ++k)
        limited_loss_values[j * n_objectives + k] = std::max(val_i[k], val_j[k]);
    }
    if (end_row_index_limited > 2)
      end_row_index_limited = _pack_pareto_sols(limited_loss_values, nondominated_indices_buf,
                                                n_objectives, end_row_index_limited);
    hv -= _compute_hypervolume(limited_loss_values, ref_point, nondominated_indices_buf,
                               n_objectives, end_row_index_limited);
  }
  return hv;
}

double compute_hypervolume(const vector<vector<double>>& sorted_pareto_sols,
                           const vector<double>& ref_point) {
  assert(sorted_pareto_sols[0].size() == ref_point.size() &&
         "The shape unmatch between ref_point and sorted_pareto_sols.");
  for (int i = 0; i < (int)sorted_pareto_sols.size() - 1; ++i) {
    assert(sorted_pareto_sols[i][0] <= sorted_pareto_sols[i + 1][0] &&
           "sorted_pareto_sols should be sorted by the first objective.");
    for (int j = 0; j < ref_point.size(); ++j)
      assert(sorted_pareto_sols[i][j] <= ref_point[j] &&
             "The Pareto solutions must be less than ref_point.");
  }
  ...
  return _compute_hypervolume(flattened_sorted_pareto_sols, ref_point.data(),
                              nondominated_indices_buf, n_objectives, n_max_trials);
}

PYBIND11_MODULE(hvcpp, m) {
  m.doc() = "Compute hypervolume in C++";
  m.def("compute_hypervolume", &compute_hypervolume, "Compute the hypervolume of a Pareto front",
        py::arg("sorted_pareto_sols"), py::arg("ref_point"));
}
